...
import static com.youssefNafaa.batch.mappers.FormateurItemPreparedStatementSetter.FORMATEURS_INSERT_QUERY;
...
import com.youssefNafaa.batch.listeners.ChargementFormateursStepListener;
import com.youssefNafaa.batch.mappers.FormateurItemPreparedStatementSetter;
...
public class ChargementFormateurStepConfig {
	...
	public Step chargementFormateursStep(final StepBuilderFactory builderFactory) {
		...
		return builderFactory.get("chargementFormateursStep").<Formateur, Formateur>chunk(10)
				.reader(formateurItemReader(null)).writer(formateurItemWriter(null))
				.listener(chargementFormateursListener()).build();
	}
