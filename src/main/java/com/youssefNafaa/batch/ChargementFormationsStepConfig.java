...
import static com.youssefNafaa.batch.mappers.FormationItemPreparedStatementSetter.FORMATIONS_INSERT_QUERY;
...
import com.youssefNafaa.batch.listeners.ChargementFormationsStepListener;
import com.youssefNafaa.batch.mappers.FormationItemPreparedStatementSetter;
...
public class ChargementFormationsStepConfig {
	...
	public JdbcBatchItemWriter<Formation> formationItemWriter(final DataSource dataSource) {
		return new JdbcBatchItemWriterBuilder<Formation>().dataSource(dataSource).sql(FORMATIONS_INSERT_QUERY)
				.itemPreparedStatementSetter(new FormationItemPreparedStatementSetter()).build();
	}
	...
	public Step chargementFormationsStep(final StepBuilderFactory stepBuilderFactory) {
		return stepBuilderFactory.get("chargementFormationsStep").<Formation, Formation>chunk(10)
				.reader(formationItemReader(null)).writer(formationItemWriter(null))
